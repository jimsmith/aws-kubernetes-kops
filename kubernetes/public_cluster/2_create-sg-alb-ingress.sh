...
--ip-permissions '[{"IpProtocol": "tcp", "FromPort": 443, "ToPort": 443, "IpRanges": [{"CidrIp": "<my-ip-here>/32", "Description": "HTTPS access from My Internet Connection"}]}]'

#HTTP
aws ec2 --region $AWS_REGION authorize-security-group-ingress --group-id $ALB_SG \
--ip-permissions '[{"IpProtocol": "tcp", "FromPort": 80, "ToPort": 80, "IpRanges": [{"CidrIp": "<my-ip-here>/32", "Description": "HTTP access from My Internet Connection"}]}]'
